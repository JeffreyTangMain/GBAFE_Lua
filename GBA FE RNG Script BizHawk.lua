...
local memoryreadword = memory.read_u16_le
local memorywriteword = memory.write_u16_le
local SHIFT = bit.arshift
local XOR = bit.bxor
local AND = bit.band
local tablesort = table.sort
local guitext = gui.text
local bitband = function(a, b) return a & b; end;
local bitbxor = function(a, b) return a ~ b; end;
local bitrshift = function(a, b) return a >> b; end;
local bitlshift = function(a, b) return a << b; end;
...
function previousSuperRN(r1, r2, r3)
	...
	local val = (0xFFFE & (r3 ~ (r2 >> 5) ~ (r1 << 11)))
	val = bitbor(val,bitband(0x0001,bitbxor(r2,(r1 >> 5))))
...
function printRNG(n)
	...
	for i=1,n do
		--guitext(232, 8*(i-1), superRNToRN(RNGTable[i]), 0xFFFF5555)
		guitext(0, 15*(i-1), superRNToRN(RNGTable[i]), nil, "topright")
	end
	--guitext(170,0,"Next RNs:", 0xFFFF5555)
end
...
function displaySearch()
	...
	if tonumber(displayTable[1]) == nil then
		guitext(0, 15, displayTable[1], "cyan")
		return
	end
	local color = "cyan"
	for i = 1, #displayTable, 1 do
		...
		guitext(xDistance, 15, displayTable[i], color)
		xDistance = xDistance + (#tostring(displayTable[i])) * 10 + 0
		if i < #displayTable then
			guitext(xDistance, 15, ",", "white")
	...
	for i=1,20 do
		if tonumber(searchFoundPositions[j]) ~= nil and searchFoundPositions[j] == RNGPosition + (i-1)  and displaySearchOn then
			if(i == 1) then
				guitext(20, 15*(i-1), "--->", "green", "topright")
			else
				guitext(20, 15*(i-1), "--->", 0xFF00B0C0, "topright")
			end
			j = j + 1
		end
	end
